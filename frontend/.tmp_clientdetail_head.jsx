...
const EXERCISE_LIBRARY = [
  ...
  'Eleva├º├úo Lateral', 'Puxada Alta', 'B├¡ceps Curl', 'Tr├¡ceps Polia',
...
function buildDefaultWorkoutDocument(client) {
  return {
    ...
    warmup: '5-10 min - Herzfrequenz langsam steigern und den K├Ârper mobil vorbereiten.',
    trainingTitle: 'Ganzk├Ârper Training 2-3x pro Woche',
    workoutRows: [],
    executionNotes: [
      'Alle ├£bungen bis zur mittleren Ausbelastung durchf├╝hren.',
      'Alle Ausf├╝hrungen ohne Schwung und kontrolliert ausf├╝hren.',
      'Pause zwischen S├ñtzen: 30 Sekunden.',
    ].join('\n'),
    nutritionNotes: client?.nutritionHabits || [
      '3-4 Mahlzeiten pro Tag mit Fokus auf Protein.',
      'Priorit├ñt bei jeder Mahlzeit: Protein -> Gem├╝se/Salat -> Kohlenhydrate.',
      '2-2,5L Wasser pro Tag und m├Âglichst nat├╝rliche Lebensmittel bevorzugen.',
...
function formatMetricValue(entry) {
  ...
  if (entry.valueBoolean !== null && entry.valueBoolean !== undefined) return entry.valueBoolean ? 'Sim' : 'N├úo';
  return entry.valueText || 'ÔÇö';
}
...
export default function ClientDetailPage() {
  ...
  const toggleOnlineStatus = async () => {
    ...
    try {
      ...
      showError(err.response?.data?.error || 'N├úo foi poss├¡vel atualizar o estado online deste cliente.');
    } finally {
  ...
  const exportComparison = () => {
    ...
    popup.document.write(`
      <html>
        ...
        <body>
          <h1>Compara├º├úo Antes / Depois</h1>
          <div class="grid">
            <div>
  ...
  const latestWeight = weightData.length > 0 ? `${weightData[weightData.length - 1].value} kg` : 'ÔÇö';
      ...
      {activeTab === 'dashboard' && (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isTablet ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: 12, marginBottom: 16 }}>
            {[
              { label: t('clientDetail.initialWeight'), val: client.initialWeight ? `${client.initialWeight} kg` : 'ÔÇö', accent: '#10253C' },
        ...
        <>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
              <div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
                  ...
                  <span style={{ padding: '6px 10px', borderRadius: 999, background: '#FFFFFF', color: '#5682B1', fontSize: 11, fontWeight: 700 }}>
                    {`${(workoutDraft.document?.workoutRows || []).filter((row) => row.exercise?.trim()).length} ├£bungen`}
                  </span>
                </div>
          ...
          <fieldset disabled={!editingWorkout} style={{ display: 'grid', gap: 12, border: 'none', padding: 0, margin: 0, minWidth: 0 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(2, minmax(0, 1fr))', gap: 12 }}>
              <div style={{ ...card, marginBottom: 0, padding: isMobile ? '18px 16px' : '20px 22px' }}>
                <div style={{ fontSize: 15, fontWeight: 800, color: '#000000', marginBottom: 14 }}>Pers├Ânliche Daten</div>
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(2, minmax(0, 1fr))', gap: 12 }}>
                  <div>
                  ...
                  <div>
                    <label style={labelStyle}>Gr├Âsse</label>
              ...
              <div style={{ ...card, marginBottom: 0, padding: isMobile ? '18px 16px' : '20px 22px' }}>
                <div style={{ fontSize: 15, fontWeight: 800, color: '#000000', marginBottom: 14 }}>Trainingsziel bis zum n├ñchsten Termin</div>
                <textarea
                  rows={4}
                ...
                />
                <div style={{ fontSize: 15, fontWeight: 800, color: '#000000', margin: '18px 0 14px' }}>Weitere Priorit├ñten / Hinweise</div>
                <textarea
                  rows={5}
                  value={workoutDraft.document?.priorities || ''}
                  onChange={(e) => updateWorkoutDocumentField('priorities', e.target.value)}
                  style={{ ...fieldStyle, resize: 'vertical', lineHeight: 1.5 }}
                  placeholder="Zus├ñtzliche medizinische oder organisatorische Hinweise"
                />
              </div>
            </div>
            ...
            <div style={{ ...card, marginBottom: 0, padding: isMobile ? '18px 16px' : '20px 22px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '0.9fr 1.1fr', gap: 12, alignItems: 'end' }}>
                ...
                <div>
                  <label style={labelStyle}>├£berschrift Trainingsblock</label>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, marginTop: 18, marginBottom: 12, flexWrap: 'wrap' }}>
                <div>
                  <div style={{ fontSize: 15, fontWeight: 800, color: '#000000' }}>Ganzk├Ârper Training</div>
                  <div style={{ fontSize: 12, color: '#739EC9', marginTop: 3 }}>├£bung, S├ñtze, Wiederholungen und Gewicht wie im neuen Planformat.</div>
                </div>
                <QuickActionButton onClick={() => addWorkoutRow()}>Zeile hinzuf├╝gen</QuickActionButton>
              </div>
              ...
              <div style={{ border: '1px solid #D6E2EF', borderRadius: 16, overflow: 'hidden' }}>
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1.1fr 2fr 0.8fr 1fr 0.9fr 44px', gap: 0, background: '#EEF4FA', color: '#5682B1', fontSize: 11, fontWeight: 800, textTransform: 'uppercase' }}>
                  ...
                  <div style={{ padding: '12px 10px' }}>├£bung</div>
                  <div style={{ padding: '12px 10px' }}>S├ñtze</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(2, minmax(0, 1fr))', gap: 12 }}>
              <div style={{ ...card, marginBottom: 0, padding: isMobile ? '18px 16px' : '20px 22px' }}>
                <div style={{ fontSize: 15, fontWeight: 800, color: '#000000', marginBottom: 14 }}>Plan Durchf├╝hrung</div>
                <textarea
                  rows={10}
              ...
              <div style={{ ...card, marginBottom: 0, padding: isMobile ? '18px 16px' : '20px 22px' }}>
                <div style={{ fontSize: 15, fontWeight: 800, color: '#000000', marginBottom: 14 }}>Ern├ñhrung / Supplementation</div>
                <textarea
                  rows={10}
            ...
            >
              <div style={{ display: 'grid', gap: 12 }}>
                <div style={{ fontSize: 12, color: '#5682B1', lineHeight: 1.55 }}>
                  Usa o mesmo princ├¡pio do intake do cliente: a AI s├│ preenche o que conseguir reconhecer no documento.
                </div>
                <input
                  type="file"
                ...
                <div style={{ padding: '12px 14px', borderRadius: 14, background: '#F4F8FC', border: '1px solid #D6E2EF', fontSize: 12, color: '#5682B1', lineHeight: 1.6 }}>
                  Campos suportados: refer├¬ncia, data, dados pessoais, objetivo, warmup, t├¡tulo do treino, linhas da tabela, notas de execu├º├úo, nutri├º├úo e mensagem final.
                </div>
              </div>
            </SlidePanel>
          ) : null}
